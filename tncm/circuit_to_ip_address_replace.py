...
# conn1 = pymysql.connect(host='localhost', port=3306, user='root', password='root', database='nc_resource', charset='utf8', autocommit=True)

# 1.读取Excel数据到集合中
df = pandas.read_excel('./电路ip地址替换模板.xlsx', names=None)
# 2.读取数据存入二维列表中
...
sql_0 = "SELECT uuid FROM `nc_resource`.`ipv4_addr` WHERE NAME = %s"
# 定义一个列表
new_cir_ip_exists = []
new_cir_ip_no = []
c = 0
...
for i in A_list:
    ...
    cursor1.execute(sql_0, i[2])
    ip_uuid = cursor1.fetchall();
    if total != 0:
        new_cir_ip_exists.append(i)
        new_cir_ip_exists[c].append(uuid[0][0])
        new_cir_ip_exists[c].append(ip_uuid[0][0])
        c = c + 1
    else:
        # 存入为查询到uuid的信息
...
for i in new_cir_ip_exists:
    sql_2 = "UPDATE circuit_to_ip_address SET ip_address_name = '%s',ip_adress_uuid = '%s' WHERE UUID = '%s';" % (i[2], i[4], i[3])
    print(sql_2, file=a)
print("###################################调度库与资源库更新电路ip地址信息############################################")
...
pandas.DataFrame(new_cir_ip_exists).to_excel(lujin, sheet_name='已更新电路IP信息', index=False, header=None)
pandas.DataFrame(new_cir_ip_no).to_excel(lujin, sheet_name='ip+电路未查询数据信息', index=False, header=None)
