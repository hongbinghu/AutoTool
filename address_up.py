...
# 读取Excel中B列A端标准地址信息
df = pandas.read_excel('C:\\Users\\HongBingHu\\Desktop\\标准地址导入模板.xlsx', usecols=[1], names=None)
# 读取数据中为空的nan替换成None
...
# 查询标准地址是否存在
sql_1 = '''SELECT NAME,UUID FROM `ne_resource`.`address` WHERE NAME in %s'''
cursor = conn3.cursor()
...
# 读取A列B列数据存入列表AB = []
df1 = pandas.read_excel('C:\\Users\\HongBingHu\\Desktop\\标准地址导入模板.xlsx', usecols=[0, 1], names=None)
# 替换Excel中的空值
...
print('-- --------------------------------------------------------------------------------------')
# 读取Excel中C列Z端标准地址信息
df2 = pandas.read_excel('C:\\Users\\HongBingHu\\Desktop\\标准地址导入模板.xlsx', usecols=[2], names=None)
# 读取数据中为空的nan替换成None
# df[pandas.isna(df)] = None
...
for i in Z_list:
    Z_result.append(i[0])
# 查询标准地址是否存在
sql_3 = '''SELECT NAME,UUID FROM `ne_resource`.`address` WHERE NAME in %s'''
cursor = conn3.cursor()
...
# 读取A列C列数据存入列表AB = []
df3 = pandas.read_excel('C:\\Users\\HongBingHu\\Desktop\\标准地址导入模板.xlsx', usecols=[0, 2], names=None)
# 替换Excel中的空值
...
ps1 = pandas.read_excel('C:\\Users\\HongBingHu\\Desktop\\标准地址导入模板.xlsx', usecols='A:C', names=None)
# 替换Excel中的空值
...
lujin = pandas.ExcelWriter('C:\\Users\\HongBingHu\\Desktop\\标准地址更新数据导出.xlsx')
...
pandas.DataFrame(ps_list).to_excel(lujin, sheet_name='基础数据', index=None)
pandas.DataFrame(new_AB).to_excel(lujin, sheet_name='A端标准地址已更新数据', index=None)
pandas.DataFrame(new_AC).to_excel(lujin, sheet_name='Z端标准地址已更新数据', index=None)
pandas.DataFrame(cp_AB).to_excel(lujin, sheet_name='A端标准地址不存数据', index=None)
pandas.DataFrame(cp_AC).to_excel(lujin, sheet_name='Z端标准地址不存在数据', index=None)
